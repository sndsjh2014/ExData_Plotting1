png(filename = "./plot3.png", width=480, height=480)
proj1 = read.table("./household_power_consumption.txt", header=TRUE, sep=";", na.strings=c("?"))
proj_sub = subset(proj1, Date=="1/2/2007" | Date=="2/2/2007")
proj_sub$DateTime<-paste(proj_sub$Date, proj_sub$Time)
# First plot  Sub_metering_1
with(proj_sub, plot(strptime(DateTime, "%d/%m/%Y %H:%M:%S"), 
                    Sub_metering_1, type="l", xlab="", 
                    ylab="Energy sub metering", main="Plot 3"))
# Now add Sub_metering_2
with(proj_sub, points(strptime(DateTime, "%d/%m/%Y %H:%M:%S"), 
                      Sub_metering_2, col="red", type="l", xlab="", 
                      ylab="Energy sub metering"))
# Then add Sub_metering_3
with(proj_sub, points(strptime(DateTime, "%d/%m/%Y %H:%M:%S"), 
                      Sub_metering_3, col="blue", type="l", xlab="", 
                      ylab="Energy sub metering"))
# Now add the legends
legend("topright", lty=1, col=c("black", "red", "blue"), 
       legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))
dev.off()